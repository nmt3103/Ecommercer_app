...
import com.example.ecommercer_app.TienApplication
...
import com.example.ecommercer_app.util.validateEmail
...
class UserAccountViewModel @Inject constructor(
    ...
    private fun saveUserInformationWithNewImage(user: User, imageUri: Uri) {
        viewModelScope.launch {
            try {
                val imageBitmap = MediaStore.Images.Media.getBitmap(getApplication<TienApplication>().contentResolver,imageUri)
    ...
    private fun saveUserInformation(user: User, shouldRetrievedOldImage: Boolean) {
        firestore.runTransaction { trasaction ->
            val documentRef = firestore.collection("user").document(auth.uid!!)
            if (shouldRetrievedOldImage){
                val currentUser = trasaction.get(documentRef).toObject(User::class.java)
                val newUser = user.copy(imagePath = currentUser?.imagePath ?: "")
                trasaction.set(documentRef,newUser)
            }else{
                trasaction.set(documentRef,user)
            }
